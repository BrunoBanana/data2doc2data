...
| 25 | Build the task-first business analysis workbench | in progress | Tasks 1–13/17 completed: immutable run history now includes event counts, failure diagnosis, stale-snapshot warnings, full event/graph replay, safe retry on current locked assets, and SQLite-backed idempotency; event playback and standalone HTML reports next |
