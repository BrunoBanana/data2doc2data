...
export function DashboardCanvas({ dashboard }: { dashboard: DashboardSpec }) {
  ...
  return (
    <section className="dashboard-canvas" aria-labelledby="dashboard-title">
      ...
      {provenance && <div className="provenance-backdrop" role="presentation" onMouseDown={() => setProvenance(null)}><section className="provenance-dialog" role="dialog" aria-modal="true" aria-label="计算依据" onMouseDown={(event) => event.stopPropagation()}><button className="icon-button provenance-close" type="button" aria-label="关闭计算依据" onClick={() => setProvenance(null)}>×</button><p className="eyebrow">PROVENANCE</p><h3>{provenance.title}</h3><dl><dt>计算</dt><dd>{provenance.provenance.expression}</dd><dt>字段</dt><dd>{provenance.provenance.fields.join('、')}</dd><dt>快照</dt><dd><code>{provenance.provenance.snapshot_id}</code></dd><dt>结果行数</dt><dd>{provenance.provenance.result_row_count}</dd></dl></section></div>}
    </section>
  )
}
