...
export default function EvidenceFlowCanvas({ visible, edges: rawEdges, activeArtifactRefs, onSelect }: { visible: EvidenceNode[]; edges: EvidenceEdge[]; activeArtifactRefs: string[]; onSelect: (nodeId: string) => void }) {
  ...
  return <div className="graph-canvas"><ReactFlow nodes={nodes} edges={edges} fitView nodesConnectable={false} nodesDraggable={false} onNodeClick={(_, node) => onSelect(node.id)}><Background color="#ded8ca" gap={32} /><Controls showInteractive={false} /></ReactFlow></div>
}
