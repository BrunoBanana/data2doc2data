...
export default function EvidenceFlowCanvas({ visible, edges: rawEdges, onSelect }: { visible: EvidenceNode[]; edges: EvidenceEdge[]; onSelect: (nodeId: string) => void }) {
  const visibleIds = new Set(visible.map((node) => node.node_id))
  const nodes: Node[] = visible.map((node, index) => ({ id: node.node_id, position: positionNode(node.kind, index), data: { label: node.label }, className: `evidence-node evidence-node--${node.status}`, draggable: false }))
