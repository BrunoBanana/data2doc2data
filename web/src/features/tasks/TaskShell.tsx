  ...
  return <>
    ...
    <div className={`workbench-grid${assistantOpen ? '' : ' workbench-grid--assistant-closed'}`}>
      ...
      <main className="analysis-canvas">
        ...
        {!loadingDashboard && activeTab === '证据' && (runResult ? <EvidenceGraph graph={runResult.evidence_graph} /> : <section className="empty-workspace" aria-labelledby="view-title"><div className="empty-visual" aria-hidden="true"><span className="empty-node" /><span className="empty-line" /><span className="empty-node empty-node--accent" /></div><p className="eyebrow">EVIDENCE</p><h2 id="view-title">运行一次可观察分析</h2><p>系统将展示计算事件、文档抽取、证据关系与假设验证状态。</p></section>)}
